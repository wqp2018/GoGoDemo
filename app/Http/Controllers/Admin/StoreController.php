...
class StoreController extends BaseController{
    ...
    public function getConfig(){
        $data['auto_refuse_order_time'] = get_db_config('auto_refuse_order_time', 30);

        return view('Admin.store.config', compact('data'));
    }

    public function postConfig(Request $request){
        $auto_refuse_order_time = intval($request->get('auto_refuse_order_time', 30));

        set_db_config('auto_refuse_order_time', $auto_refuse_order_time, "订单自动取消时间");

        return $this->ajaxSuccess('', '修改成功', url('Store/config'));
    }
}